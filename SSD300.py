...
#from L2_normaliz import L2Normalization
#--------------------------
from SSD_layers import PriorBox

def SSD300v2(input_shape, num_classes=6, featurte_map=None):
    """SSD300 architecture.
    # Arguments
    ...
    with tf.name_scope("conv4_3"):
        #-------Rewritten--------
        conv4_3_norm = BatchNormalization(axis=1, name='conv4_3_norm')(conv4_3)
        #------------------------
    ...
    with tf.name_scope("pool6"):
        ...
        pool6_mbox_priorbox = PriorBox(img_size, 276.0, max_size=330.0, aspect_ratio=[2, 3],
                                       variances=[0.1, 0.1, 0.2, 0.2],
    ...
    with tf.name_scope("mbox"):
        ...
        mbox_priorbox = concatenate([conv4_3_norm_mbox_priorbox,
                                    ...
                                    axis=1,
                                    name='mbox_priorbox')
        #Rewritten all _keras_shape -> K.int_shape()
        print('{} conv4_3_norm_mbox_loc_flat'.format(K.int_shape(conv4_3_norm_mbox_loc_flat)))
        print('{} conv4_3_norm_mbox_conf_flat'.format(K.int_shape(conv4_3_norm_mbox_conf_flat)))
        ...
        print('{} mbox_loc'.format(K.int_shape(mbox_loc)))
        print('{} mbox_conf'.format(K.int_shape(mbox_conf)))
        mbox_loc = Reshape((num_boxes, 4),
                           name='mbox_loc_final')(mbox_loc)
        #------add----------
        mbox_priorbox = Reshape((num_boxes, 4),
                                 name='mbox_priorbox_final')(mbox_priorbox)
        #-------------------
