...
class BBoxUtility(object):
  def __init__(self, num_classes, priors=None, overlap_threshold=0.5,
    ...
    self.boxes = None
    self.scores = None
    # self.boxes = tf.compat.v1.placeholder(tf.float32, shape=(None, 4))
    # self.scores = tf.compat.v1.placeholder(tf.float32, shape=(None,))
    # BBox sort socore descending oder
  ...
  # How to avoid using of placeholder, session
  @tf.function
  def bbox_sort_socore_desc(self):
  ...
  def decode_boxes(self, mbox_loc, mbox_priorbox, variances):
    ...
    decode_bbox_center_x = mbox_priorbox[:, 0] * prior_width * variances[:, 0]
    decode_bbox_center_x += prior_center_x
    decode_bbox_center_y = mbox_priorbox[:, 1] * prior_width * variances[:, 1]
    ...
    decode_bbox = np.minimum(np.maximum(decode_bbox_ymax, 0.0), 1.0)
    return decode_bbox
    ...
    for i in range(len(mbox_loc)):
      ...
      for c in range(self.num_classes):
        ...
        if len(c_confs[c_confs_m]) > 0:
          ...
          self.scores = confs_to_process
          idx = self.bbox_sort_socore_desc()
          # -------------------------------
